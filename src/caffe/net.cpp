...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
    ...
    // Fix to ensure DataReader will know UID of root net:
    // 		All DataLayers use DataReader that will share data source (i.e. use internal static queues) among
    //      data layers that have the same name (layer_param.name()) and the same
    //      path to data (layer_param.data_param().source()). This works well for multi-GPU in executable to share underlaying data
