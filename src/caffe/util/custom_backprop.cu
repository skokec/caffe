...
/*
	if (kernel_width == 3 && kernel_height == 3 && F % 4 == 0 && G % 4 == 0) {
		dim3 numBlocks = calculateBlocks(threadsPerBlock, error_width, error_height, S, F, G, BATCH_PIXELS_SIZE, 4, 4);
		if (error_width % (BLOCK_X*BATCH_PIXELS_SIZE) == 0)
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,3,3,4,4,0><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);
		else
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,3,3,4,4,1><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);

	} else if (kernel_width == 3 && kernel_height == 3 && F % 4 == 0 && G % 3 == 0) {

		dim3 numBlocks = calculateBlocks(threadsPerBlock, error_width, error_height, S, F, G, BATCH_PIXELS_SIZE, 4, 3);
		if (error_width % (BLOCK_X*BATCH_PIXELS_SIZE) == 0)
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,3,3,4,3,0><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);
		else
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,3,3,4,3,1><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);

	} else if (kernel_width == 3 && kernel_height == 3 && F % 8 == 0 && G % 4 == 0) {

		dim3 numBlocks = calculateBlocks(threadsPerBlock, error_width, error_height, S, F, G, BATCH_PIXELS_SIZE, 8, 4);
		if (error_width % (BLOCK_X*BATCH_PIXELS_SIZE) == 0)
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,3,3,8,4,0><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);
		else
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,3,3,8,4,1><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);

	} else if (kernel_width == 5 && kernel_height == 5 && F % 8 == 0 && G % 4 == 0) {

		dim3 numBlocks = calculateBlocks(threadsPerBlock, error_width, error_height, S, F, G, BATCH_PIXELS_SIZE, 8, 4);
		if (error_width % (BLOCK_X*BATCH_PIXELS_SIZE) == 0)
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,5,5,8,4,0><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);
		else
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,5,5,8,4,1><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);

	} else if (kernel_width == 7 && kernel_height == 7 && F % 8 == 0 && G % 4 == 0) {

		dim3 numBlocks = calculateBlocks(threadsPerBlock, error_width, error_height, S, F, G, BATCH_PIXELS_SIZE, 8, 4);
		if (error_width % (BLOCK_X*BATCH_PIXELS_SIZE) == 0)
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,7,7,8,4,0><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);
		else
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,7,7,8,4,1><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);

	} else if (kernel_width == 9 && kernel_height == 9 && F % 8 == 0 && G % 4 == 0) {

		dim3 numBlocks = calculateBlocks(threadsPerBlock, error_width, error_height, S, F, G, BATCH_PIXELS_SIZE, 8, 4);
		if (error_width % (BLOCK_X*BATCH_PIXELS_SIZE) == 0)
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,9,9,8,4,0><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);
		else
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,9,9,8,4,1><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);

	} else if (kernel_width == 11 && kernel_height == 11 && F % 8 == 0 && G % 4 == 0) {

		dim3 numBlocks = calculateBlocks(threadsPerBlock, error_width, error_height, S, F, G, BATCH_PIXELS_SIZE, 8, 4);
		if (error_width % (BLOCK_X*BATCH_PIXELS_SIZE) == 0)
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,11,11,8,4,0><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);
		else
			filterActs_YxX_color_kernel<BLOCK_X,BLOCK_Y,11,11,8,4,1><<<numBlocks,threadsPerBlock,0,streamId>>>(images, error, filters, output, I, S, F, G, subfeat_i, feat_i, gauss_i, img_width, img_height, error_width, error_height, kernel_width, kernel_height, padding, stride);

	} else {
		LOG(ERROR) << "no predefined kernel in custom_backprop.cu for kernel [" << kernel_width << " x " << kernel_height << "], num features " << F << ", num guass " << G ;
		throw std::exception();
	}*/
}
