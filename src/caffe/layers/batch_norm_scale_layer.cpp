...
void BatchNormScaleLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  BatchNormScaleParameter param = this->layer_param_.batch_norm_scale_param();
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > scale_filler(
      GetFiller<Dtype>(this->layer_param_.batch_norm_scale_param().scale_filler()));
    scale_filler->Fill(this->blobs_[0].get());
    shared_ptr<Filler<Dtype> > bias_filler(
      GetFiller<Dtype>(this->layer_param_.batch_norm_scale_param().bias_filler()));
...
template <typename Dtype>
void BatchNormScaleLayer<Dtype>::multicast_cpu(int N, int C, int S,
...
//  y[c] = sum x(.,c,...)
template <typename Dtype>
void BatchNormScaleLayer<Dtype>::compute_sum_per_channel_cpu(int N, int C, int S,
    const Dtype *x, Dtype *y ) {
  Blob<Dtype> temp_NC;
  vector<int> temp_size;
  temp_size.push_back(N*C);
...
// y[c] = mean x(.,c,...)
template <typename Dtype>
void BatchNormScaleLayer<Dtype>::compute_mean_per_channel_cpu(int N, int C, int S,
    const Dtype *x, Dtype *y) {
  Dtype F = 1.0 / (N*S);
