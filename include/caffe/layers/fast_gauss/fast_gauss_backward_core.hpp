...
//#define PROFILE_CUDA
		...
		for (int jj = 0; jj < 1; ++jj) {
			...
			clock_t start_t = clock();
#endif
            fast_gauss_backward_multi_pipeline_kernel<BlockIndexingPipelineT><<<numBlocks,threadsPerBlock, 0, p.streamId>>>(p.prepared_filtered_images, p.prepared_error_images, p.prepared_filter_offsets, p.prepared_filter_weights, p.output, I * new_img_parts_width * new_img_parts_height, S, F, G, K, img_width, img_height);
#ifdef PROFILE_CUDA
            cudaDeviceSynchronize();
...
#define RUN_KERNEL_R6(CLASS_NAME, IMG_PATCH_SIZE_W, IMG_PATCH_SIZE_H, MAX_OFFSET, SMALLER_WARP_AND_GROUP_K, BATCH_IMAGES, USE_INTERPOLATION, SINGLE_SUBFEATURE, PARAMS, ...) \
	if (IMG_PATCH_SIZE_W >= 64) { \
        ...
        RUN_KERNEL_R5(CLASS_NAME, 32, IMG_PATCH_SIZE_H, MAX_OFFSET, SMALLER_WARP_AND_GROUP_K, BATCH_IMAGES, USE_INTERPOLATION, SINGLE_SUBFEATURE, PARAMS, __VA_ARGS__) \
    } else { \
